...
\begin{exoframe}{Exercice}
    \begin{block}{Analysez les énoncés suivants}
        \begin{itemize} \color{white}
            \item Ça fait du bien quand on est lessivés \hfill \only<2->{$\Rightarrow$ lessive}
            \item T'es vraiment un chips type \hfill \only<2->{$\Rightarrow$ chips}
            \item Vous allez avoir des papillons dans le ventre \hfill \only<2->{$\Rightarrow$ pâtes farfalles}
            \item \textit{It's raining nems} \hfill \only<2->{$\Rightarrow$ nems}
            \item On est champignons d'Europe \hfill \only<2->{$\Rightarrow$ champignons}
        \end{itemize}
    \end{block}
\end{exoframe}
